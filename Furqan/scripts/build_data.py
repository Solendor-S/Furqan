...
for s in range(1, 115):
    ...
    for (a, txt) in hverses:
        ...
        for name, h2x, xflat in (("warsh", h2w, wflat), ("abuamr", h2d, dflat)):
            arr, ayas = [], []
        ...
        abuamr_only = []
        for e in abuamr_by_verse.get((s, a), []):
            i = e["idx"]
            if not (0 <= i < len(htoks)) or not abuamr_arr[i] or i in rd:
                continue
            if skel(abuamr_arr[i], keep_dagger=True) == skel(hafs_arr[i], keep_dagger=True):
                print(f"  WARN {s}:{a} idx {i}: Abū ʿAmr does not differ from Ḥafṣ "
                      f"({abuamr_arr[i]!r}) — check farsh-abuamr.json")
                continue
            abuamr_only.append(i)
            meaning[str(i)] = {"shared": e["shared"], "cite": ABUAMR_CITE,
                               "abuamr": {"ar": abuamr_arr[i], "gloss": e["abuamr"]}}
        abuamr_only = sorted(abuamr_only)
        v = {
            "hafs_aya": a,
            "warsh_aya": warsh_ayas[0] if warsh_ayas else a,
            "abuamr_aya": abuamr_ayas[0] if abuamr_ayas else a,
            "hafs": hafs_arr, "warsh": warsh_arr, "abuamr": abuamr_arr,
            "reading": rd, "ortho": ortho, "abuamr_only": abuamr_only,
